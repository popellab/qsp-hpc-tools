...
`qsp_sim` binary (built by a consumer repo such as SPQSP_PDAC or
pdac-build from C++ sources emitted by the `qsp-codegen` package)
against a per-sim parameter XML; reads the v2 raw-binary trajectory
(species + compartments + assignment-rule values) into a MATLAB-
compatible Parquet so downstream caching / test-stat derivation works
unchanged.
...
**Location:** `qsp_hpc/cpp/` (Python). The `qsp_sim` binary lives in
a consumer repo (e.g. SPQSP_PDAC on branch `cpp-sweep-binary-io`, or
pdac-build); C++ sources are generated by the standalone `qsp-codegen`
package (SBML → `ODE_system.cpp` / `QSPParam.cpp` / param-XML snippet,
merged via `qsp-refresh-param-xml`).
