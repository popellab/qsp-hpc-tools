...
`qsp-hpc-tools` occupies a different niche: it exposes a QSP model—whether a generated C++ simulator or an existing MATLAB model—as a Python callable with content-aware caching, multi-scenario support, and transparent HPC orchestration, without requiring researchers to manage job submission scripts. The package originated as a bridge to established MATLAB/SimBiology models (still supported through the legacy `QSPSimulator` backend), but its primary path is now the generated C++ backend, which delivers order-of-magnitude speedups while preserving the same calibration-target and inference interfaces. It is currently used in production for pancreatic cancer QSP model calibration, where it manages multi-scenario simulation campaigns of thousands of virtual patients across SLURM clusters, feeding directly into neural posterior estimation pipelines.
...
For models with an expensive pre-treatment burn-in phase, the C++ backend additionally caches the post-evolve ODE state in an `evolve_cache` keyed by parameter set and model configuration, allowing multiple downstream therapeutic scenarios to skip the shared burn-in integration. The cache is a directory of append-only shards—each SLURM array task or local batch writes its own immutable shard, with no shared writable store or cross-task locking—which keeps it safe on the NFS-backed parallel filesystems typical of HPC clusters. On models where burn-in dominates wall time, this delivers near-linear speedups in the number of scenarios sharing a configuration.
...
## Pluggable backends: generated C++ (primary) and legacy MATLAB

The package supports two interchangeable simulation backends that share the cache, scheduler, and result-loading machinery. The primary backend, `CppSimulator`, drives a generated C++ ODE simulator (`qsp_sim`) built from an SBML model description by the companion `qsp-codegen` package [@qsp_codegen], which is pip-installed on the cluster and invoked automatically by `ensure_cpp_binary` to drive the in-repo CMake build. It yields roughly 25–87× per-simulation speedups over the MATLAB path on representative pancreatic cancer models. The legacy `QSPSimulator` backend wraps existing MATLAB/SimBiology models, requiring only a model script and a structure file describing species, units, and initial conditions. Both backends share the identical calibration-target interface, so existing MAPLE-style YAMLs and downstream inference code work unchanged across either path and migration from MATLAB to C++ carries no rewrite cost.
...
The primary backend is the generated C++ simulator, driven through the same
cached three-tier interface:
...
sim = CppSimulator(
    ...
    calibration_targets="calibration_targets/treatment/",  # directory of MAPLE YAMLs
...
The legacy MATLAB backend exposes the same callable interface for groups
working from an existing SimBiology model:
