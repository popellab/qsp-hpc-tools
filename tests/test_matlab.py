...
@pytest.mark.matlab
def test_matlab_tests_run_successfully(matlab_test_results):
    """
    Meta-test: verify that MATLAB tests ran and produced results.

    This test passes if MATLAB tests executed without errors,
    regardless of individual test outcomes.
    """
    assert len(matlab_test_results) > 0, "No MATLAB test results found"

    # Count results
    passed = sum(1 for r in matlab_test_results if r["passed"] is True)
    failed = sum(1 for r in matlab_test_results if r["passed"] is False)
    skipped = sum(1 for r in matlab_test_results if r["passed"] is None)

    print(f"\nMATLAB Test Summary: {passed} passed, {failed} failed, {skipped} skipped")

    # List any failures
    failures = [r for r in matlab_test_results if r["passed"] is False]
    if failures:
        print("\nFailed tests:")
        for f in failures:
            print(f"  - {f['classname']}.{f['name']}: {f['message']}")


@pytest.mark.matlab
def test_all_matlab_tests_pass(matlab_test_results):
    """
    Aggregate test: fail if any MATLAB test failed.

    This provides a single pytest failure point for all MATLAB tests.
    """
    failures = [r for r in matlab_test_results if r["passed"] is False]

    if failures:
        failure_msgs = [f"{r['classname']}.{r['name']}: {r['message']}" for r in failures]
        pytest.fail("MATLAB tests failed:\n" + "\n".join(failure_msgs))
